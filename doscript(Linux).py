#!/usr/bin/env python3
r"""
DoScript v0.6.15-linux
Changes (Linux port):
- Fix: path add on Linux/macOS now actually writes to shell profiles
  (~/.bashrc, ~/.zshrc, ~/.profile, ~/.bash_profile) instead of only logging a hint.
  Includes a marker comment so path remove can find and clean up the entry.
- Fix: path remove on Linux/macOS now actually removes the export line from all
  shell profiles where it was added by path add.
- Fix: shutdown now works cross-platform — uses `shutdown -h now` (or `+N` minutes)
  on Linux/macOS instead of raising a Windows-only error.
- No change: registry commands remain Windows-only (they error clearly on Unix).
- No change: make shortcut on Linux already creates .desktop files correctly.
- No change: notify on Linux already uses notify-send with a terminal fallback.
- No change: schedule on Linux already uses crontab correctly.
    ...
    def _exec_statement(self, stmt: str) -> Optional[Tuple[str, Any]]:
        ...
        if stmt.startswith('path add '):
            ...
            if self.dry_run:
                ...
                if sys.platform == 'win32':
                    ...
                    # Linux / macOS — write the export line into the user's shell profiles
                    export_line = f'export PATH="$PATH:{resolved}"'
                    marker = f'# doscript:path:{resolved}'
                    profiles = []
                    home = os.path.expanduser('~')
                    for name in ('.bashrc', '.zshrc', '.profile', '.bash_profile'):
                        ...
                        if os.path.exists(p):
                            profiles.append(p)
                    if not profiles:
                        # None exist yet — create .profile
                        profiles = [os.path.join(home, '.profile')]
                    added_to = []
                    for prof in profiles:
                        try:
                            try:
                                content = open(prof, 'r', encoding='utf-8').read()
                            except FileNotFoundError:
                                content = ''
                            if resolved in content:
                                self.log_info(f"Path already present in {prof}")
                                continue
                            with open(prof, 'a', encoding='utf-8') as fh:
                                fh.write(f'\n{export_line}  {marker}\n')
                            added_to.append(prof)
                        except PermissionError:
                            self.raise_error(ProcessError, f"Permission denied writing to {prof}")
                        except Exception as e:
                            self.raise_error(ProcessError, f"Failed to update {prof}: {e}")
                    if added_to:
                        self.log_info(f"Added to PATH in: {', '.join(added_to)}")
                        self.log_info(f"Run 'source {added_to[0]}' or open a new terminal for the change to take effect.")
                    else:
                        self.log_info(f"Path already present in all shell profiles — no change made.")
            return None
        ...
        if stmt.startswith('path remove '):
            ...
            if self.dry_run:
                ...
                if sys.platform == 'win32':
                    ...
                    # Linux / macOS — remove the export line from shell profiles
                    marker = f'# doscript:path:{resolved}'
                    home = os.path.expanduser('~')
                    profiles = []
                    for name in ('.bashrc', '.zshrc', '.profile', '.bash_profile'):
                    ...
                    for prof in profiles:
                        try:
                            lines_in = open(prof, 'r', encoding='utf-8').readlines()
                            # Keep lines that don't reference this path
                            new_lines = [l for l in lines_in
                                         if resolved not in l and marker not in l]
                            if len(new_lines) != len(lines_in):
                                with open(prof, 'w', encoding='utf-8') as fh:
                                    fh.writelines(new_lines)
                                removed_from.append(prof)
                        except PermissionError:
                            self.raise_error(ProcessError, f"Permission denied writing to {prof}")
                        except Exception as e:
                            self.raise_error(ProcessError, f"Failed to update {prof}: {e}")
                    if removed_from:
                        self.log_info(f"Removed PATH entry from: {', '.join(removed_from)}")
                        self.log_info("Run 'source ~/.bashrc' or open a new terminal for the change to take effect.")
                    else:
                        self.log_info(f"Path '{resolved}' not found in any shell profile — no change made.")
            return None
        ...
        if stmt.startswith('shutdown'):
            ...
            if sys.platform == 'win32':
                ...
                # Linux / macOS
                if self.dry_run:
                    self.log_dry("shutdown (Unix) - dry run")
                else:
                    try:
                        if args and args.isdigit():
                            # args in seconds — convert to minutes for shutdown
                            minutes = max(1, int(args) // 60)
                            subprocess.run(['shutdown', '-h', f'+{minutes}'], check=True)
                        else:
                            subprocess.run(['shutdown', '-h', 'now'], check=True)
                        self.log_info("Shutdown command executed (Unix).")
                    except PermissionError:
                        self.raise_error(ProcessError, "shutdown requires root/sudo on Unix. Run as root or prefix with sudo.")
                    except FileNotFoundError:
                        self.raise_error(ProcessError, "shutdown command not found on this system.")
                    except Exception as e:
                        self.raise_error(ProcessError, f"Failed to shutdown: {e}")
            return None
