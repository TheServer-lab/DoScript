# DoScript IDE companion for DoScript v0.6.9
...
KEYWORDS = [
    'function','end_function','if','else_if','else','end_if','loop','end_loop','repeat','end_repeat',
    'for_each','end_for','for_each_line','make','include','run','execute_command','execute','capture',
    'say','ask','pause','wait','return','break','continue','try','catch','end_try',
    'global_variable','local_variable','script_path','path','copy','move','delete','ping','kill','log','warn','error',
    'read_content','if_ends_with','if_file_contains','if_file_not_contains',
    'length','trim','lower','upper','replace','split','contains','contains_case','notcontains',
    'true','false'
]
...
class DoScriptIDE:
    ...
    def on_return(self, event=None):
        ...
        block_keywords = ('if ', 'else_if ', 'else', 'function ', 'loop ', 'repeat ', 'for_each ', 'for_each_line ', 'try')
        dedent_keywords = ('else_if ', 'else', 'catch')
        if stripped.startswith(dedent_keywords) and len(indent) >= 4:
            indent = indent[:-4]
        extra = ''
        if stripped:
            for bk in block_keywords:
                if stripped.startswith(bk):
                    extra = '    '
                    break
