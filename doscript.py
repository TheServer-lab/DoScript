...
r"""
v0.6.16
- changed extension from .do to .ds
DoScript v0.6.15
Changes:
...
def _download_remote_script(script_url: str, timeout: int = 30) -> Tuple[str, str]:
    ...
    basename = os.path.basename(parsed.path) or 'remote_script.ds'
    if not basename.lower().endswith('.ds'):
        basename += '.ds'
...
class DoScriptInterpreter:
    ...
    def _exec_statement(self, stmt: str) -> Optional[Tuple[str, Any]]:
        ...
        if stmt.startswith('run_from_web '):
            ...
            # Ensure .do extension
            if not script_name.endswith('.ds'):
                script_name += '.ds'
        ...
        if stmt.startswith('use_module '):
            ...
            mod_name  = self.extract_string(file_expr)
            # Ensure .do extension
            if not mod_name.endswith('.ds'):
                mod_name = mod_name + '.ds'
...
""")
        ...
        # command-style function call: funcName arg1, arg2 (no parentheses)
        # Also handles namespaced: module.func arg1, arg2
        cmd_m = re.match(r'^([\w]+(?:\.[\w]+)*)\s+(.*)', stmt)
        if cmd_m:
            fname = cmd_m.group(1)
            if fname in self.functions:
                args_raw = cmd_m.group(2).strip()
                args = [self.evaluate_expression(a.strip()) for a in self._split_args(args_raw)] if args_raw else []
                self.call_function(fname, args)
                return None
...
def _do_install_module(argv):
    ...
    filename = mod_name if mod_name.endswith(".ds") else mod_name + ".ds"
