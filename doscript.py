...
class DoScriptInterpreter:
    ...
    def _exec_statement(self, stmt: str) -> Optional[Tuple[str, Any]]:
        ...
        # ── subscript write: cfg["key"] = val  /  cfg["a"]["b"] = val ─────────
...
def _do_build(argv):
    ...
    doscript build <script.do> [options]
      --onefile       single portable EXE (PyInstaller --onefile)
      --windowed      no console window
      --icon <file>   application icon (.ico)
      --title <text>  banner title (default: script filename stem)
      --output <dir>  output directory (default: ./dist)
    """
    if not argv:
        print("Usage: doscript build <script.do> [--onefile] [--windowed]"
              " [--icon app.ico] [--title <title>] [--output dist]",
              file=sys.stderr)
        sys.exit(1)
    ...
    while i < len(argv):
        a = argv[i]
        if   a == '--onefile':                  onefile  = True
        elif a == '--windowed':                 windowed = True
        elif a == '--icon'   and i+1 < len(argv): icon   = argv[i+1]; i += 1
        elif a == '--title'  and i+1 < len(argv): title  = argv[i+1]; i += 1
        elif a == '--output' and i+1 < len(argv): out_dir= argv[i+1]; i += 1
        elif not a.startswith('--') and script_path is None: script_path = a
        else:
            print(f"Unknown build option: {a}", file=sys.stderr); sys.exit(1)
        i += 1

    if script_path is None:
        print("Error: no script specified.", file=sys.stderr); sys.exit(1)
    if not os.path.exists(script_path):
        print(f"Error: script not found: {script_path}", file=sys.stderr); sys.exit(1)
    ...
    # locate the launcher template (ships alongside doscript.py)
    tpl_path = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                            '_runtime_launcher_template.py')
    if not os.path.exists(tpl_path):
        print(f"Error: launcher template not found: {tpl_path}", file=sys.stderr)
        print("Make sure _runtime_launcher_template.py is in the same folder as doscript.py.",
              file=sys.stderr)
        sys.exit(1)
    ...
    # write launcher to a temp dir and call PyInstaller
    import tempfile
    ...
    try:
        ...
        print(f"[BUILD] Calling PyInstaller...")

        cmd = [sys.executable, '-m', 'PyInstaller', launcher_file,
               '--name', exe_name,
               '--distpath', abs_out,
        ...
        if icon:     cmd += ['--icon', os.path.abspath(icon)]

        try:
            subprocess.run(cmd, check=True)
        except FileNotFoundError:
            print("\n[BUILD ERROR] PyInstaller not found. Install it with:", file=sys.stderr)
            print("  pip install pyinstaller", file=sys.stderr)
            sys.exit(1)
        except subprocess.CalledProcessError as e:
            print(f"\n[BUILD ERROR] PyInstaller failed (exit {e.returncode})", file=sys.stderr)
            sys.exit(1)

        ext = '.exe' if sys.platform == 'win32' else ''
        out_exe = os.path.join(abs_out, exe_name + ext)
        print(f"\n[BUILD] Done!")
        print(f"[BUILD] Output: {out_exe}")
    finally:
