...
These built-ins are useful in assignments, conditions, and function calls:

```
global_variable = rawName, cleaned, loud, shortName

rawName = "  Alice Smith  "
cleaned = trim(rawName)
loud = upper(cleaned)
shortName = replace(cleaned, " Smith", "")

say 'Length: {length(cleaned)}'
say 'Lower: {lower(cleaned)}'
say 'Upper: {loud}'
say 'Short: {shortName}'
```
...
Use `execute_command` when you want to run a command without routing
through the shell:
...
This is safer than `run` when command parts come from variables or user input.
