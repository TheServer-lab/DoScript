...
Seven read-only variables are now pre-populated at startup with the correct
paths for the current user and operating system. No declaration needed —
they work anywhere in a script, including inside interpolated strings:

```
download "https://example.com/installer.do" to '{downloads}\installer.do'
make folder '{appdata}\MyApp'
make file '{temp}\run.log' with "Started"
say 'Running as: {username}'
```
...
These variables are read-only. Attempting to assign to them raises an error,
the same way built-in time variables do. Scripts using these variables are
fully portable across machines and operating systems — no more hardcoded
usernames in paths.
