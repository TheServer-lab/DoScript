...
**With a remote script URL:**
```bash
python doscript.py https://example.com/script.do
```

> Remote scripts are downloaded to a temporary cache and then executed like a
> normal local `.do` file. Relative `include` paths inside remote scripts are
> still resolved from the downloaded local copy.
...
In DoScript v0.6.9, you also have:

```
global_variable = cleaned, loud, chars

cleaned = trim("  hello  ")
loud = upper(cleaned)
chars = length(cleaned)
```
...
Use `execute_command` when you want to run a program without going through
the shell:
...
Use `run` only when you specifically want shell behavior or a single shell
command string.
